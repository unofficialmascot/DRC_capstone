...
async function run() {
  const sql = fs.readFileSync('./migrations/0002_normalize_users_schema.sql', 'utf8');
  const client = await pool.connect();
  try {
    console.log('Running migration 0002_normalize_users_schema.sql...');
    await client.query('BEGIN');
    // Execute migration file; multiple statements are allowed
    ...
    // Counts for core tables if present
