...
function SupervisorApplications({ user }: { user: User }) {
  const [selectedApp, setSelectedApp] = useState<Application | null>(null);
  const { data: applications = [], isLoading } = useQuery<Application[]>({
    queryKey: ["/api/applications/supervisor"],
    queryFn: async () => {
      const res = await fetch("/api/applications/supervisor");
      if (!res.ok) throw new Error("Failed to fetch supervisor applications");
      return res.json();
    }
  });
...
function EnclosureUpload({ onChange }: { onChange: (enclosures: ApplicationEnclosure[]) => void }) {
  ...
  const handleFiles = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + pdfFiles.length > 10) {
      alert("You can upload up to 10 PDF enclosures per application.");
      event.target.value = "";
      return;
    }

    const oversized = pdfFiles.find((file) => file.size > 5 * 1024 * 1024);
    if (oversized) {
      alert(`"${oversized.name}" exceeds the 5MB limit.`);
      event.target.value = "";
      return;
    }
  ...
  return (
    <div className="form-group">
      ...
      {files.length > 0 && (
        <ul style={{ marginTop: "8px", fontSize: "13px", paddingLeft: "18px" }}>
          {files.map((file, i) => (
            <li key={`${file.name}-${i}`} style={{ marginBottom: "6px" }}>
              {file.name}
              <button
                type="button"
                onClick={() => {
                ...
                style={{ marginLeft: "8px", border: "none", background: "transparent", color: "#c0392b", cursor: "pointer" }}
              >
                Remove
              </button>
            </li>
